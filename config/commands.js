...
var commands = exports.commands = {
	...
	poke: function (target, room, user) {
		if (!target) return this.sendReply('/poke [target] - Pokes the target.');
		return this.parse('/me pokes ' + target + ' with a backrubber.');
	},

	slap: function (target, room, user) {
		if (!target) return this.sendReply('/slap [target] - Slaps the target.');
		return this.parse('/me slaps ' + target + ' in the face with a slipper!');
	},

	s: 'spank',
	spank: function (target, room, user) {
		if (!target) return this.sendReply('/spank [target] - Spanks the target.');
	...
	magazine: 'theplayer',
	webzine: 'theplayer',
	theplayer: function (target, room, user) {
		if (!this.canBroadcast()) return;
		this.sendReplyBox(
			"Pokemon Showdown!'s official webzine is <a href=\"https://www.smogon.com/player\">The Player</font>! It is mainly focused on:<br />" +
			"- Bringing the most recent <a href=\"https://www.smogon.com/player/issue1/recently-on-ps\">news</a> to the users in a more fashionable way<br />" +
	...
	roomhelp: function (target, room, user) {
		...
		this.sendReplyBox(
			...
			"- /mute, /hourmute OR /m, /hm <em>username</em>: 7 minute mute or 60 minute mute respectively<br />" +
			...
			"- /roomvoice OR /roomdevoice <em>username</em>: appoint or remove a room voice<br />" +
			...
			"- /roommod, /roomdriver OR /roomdemod, /roomdedriver <em>username</em>: appoint or remove a room moderator/driver<br />" +
