...
var commands = exports.commands = {
	...
	bank: function (target, room, user) {
		if (cmd === 'bank' && !user.can('roommod', room)) {
			var setBank = recentBank;

			return this.sendReplyBox(room.title + ' \'s currently set bank is: ' + setBank + '.');
		}
		if (cmd === 'bank' && user.can('roommod', room)) {
			if (!target) return this.sendReply('/bank - No user specified.');
			target = target.toLowerCase();
			target = target.trim();

			return this.sendReply(target + ' was successfully set as the room\'s bank.');
			target = recentBank;
		}
	},
	...
	opmute: function (target, room, user) {
		...
		if (!this.can('opmute', targetUser, room)) return false;
		if (targetUser.mutedRooms[room.id] || targetUser.locked || !targetUser.connected) {
			var problem = " but was already " + (!targetUser.connected ? "offline" : targetUser.locked ? "locked" : "muted");
			if (!target) {
				return this.privateModCommand("(" + targetUser.name + " would be muted by " + user.name + problem + ".)");
	...
	gdeclare: function (target, room, user, connection, cmd) {
		...
		if (cmd === 'gdeclare') {
			for (var id in Rooms.rooms) {
				if (id !== 'global') Rooms.rooms[id].addRaw('<div class="broadcast-blue"><font size=1><i>Global declare by ' + user.name + '<br /></i></font><b>' + target + '</b></div>');
			}
		}
		if (cmd === 'gdeclarered') {
			for (var id in Rooms.rooms) {
				if (id !== 'global') Rooms.rooms[id].addRaw('<div class="broadcast-red"><font size=1><i>Global declare by ' + user.name + '<br /></i></font><b>' + target + '</b></div>');
			}
		}
		else if (cmd === 'gdeclaregreen') {
			for (var id in Rooms.rooms) {
				if (id !== 'global') Rooms.rooms[id].addRaw('<div class="broadcast-green"><font size=1><i>Global declare by ' + user.name + '<br /></i></font><b>' + target + '</b></div>');
			}
		}
	...
	forcelogout: function(target, room, user) {
		...
		if (targetUser.name === 'macrarazy') return this.sendReply('You cannot force logout another Admin.');
